...
function addAIMessage(text, side) {
...
function generateAIResponse(query) {
    ...
    // Help search
    if (q.includes('dilution')) {
        return "Pour la <b>dilution</b>, tu dois utiliser une fiole jaugée et une pipette. Le facteur est F = Cm/Cf. Regarde le protocole 'Dilution' dans l'onglet dédié ! 🧪";
    }
    if (q.includes('mole') || q.includes('matière')) {
        return "La <b>quantité de matière (n)</b> se calcule avec n = m/M pour les solides. Veux-tu que je t'affiche le chapitre 'Mole' ? 📚";
    }
    if (q.includes('couleur') || q.includes('optique')) {
        return "En <b>Optique</b>, n'oublie pas la barre algébrique sur les distances ! Relation de conjugaison : 1/OA' - 1/OA = 1/OF'. 🕯️";
    }

    // Default search in data
    const match = formulas.find(f => f.title.toLowerCase().includes(q) || f.definition.toLowerCase().includes(q));
    if (match) {
        return `J'ai trouvé quelque chose pour toi : <b>${match.title}</b>. Elle dit : ${match.definition.substring(0, 100)}...`;
    }

    return "Je ne suis pas sûr de comprendre... Essaie de me demander une formule ou un protocole précis ! 😊";
}
